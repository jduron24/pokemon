...
int getsCostForRival(int i, int j, map_t *map){
        ...
        else if(map->map[i][j] == TALL_GRASS ){
            return 20;
        }
        else{
            return INT_MAX;
...
void dijkstra(map_t *map,int x, int y, int switc) {
    ...
    // if its a tree, grass .. add the distance to its current positon
        ...
        //else if(graph[node].distance > new_distance){
...
int terrain(){
    ...
    dijkstra(&(w.map[x][y]),location.x, location.y,1);
    printf("\n");
    dijkstra(&(w.map[x][y]),location.x, location.y,0);
